...
from gaussian_renderer import GaussianModel, ManoGaussianModel
...
class LocalViewer(Mini3DViewer):
    def __init__(self, cfg: Config):
        ...
        # MANO parameters
        if self.gaussians.binding is not None:
            print("Initializing MANO parameters...")
            self.reset_mano_param()
        ...
        if self.gaussians.binding is not None:
            ...
            self.gaussians.select_mesh_by_timestep(self.timestep, zero_origin=True)

    def init_gaussians(self):
        # load gaussians
        if (Path(self.cfg.point_path).parent / "mano_param.npz").exists():
    ...
    def export_trajectory(self):
        ...
        for i in range(self.num_record_timeline):
            ...
            if timestep_index in tid2paths:
                ...
                frame['mano_param_path'] = tid2paths[timestep_index]['mano_param_path']
    ...
    def define_gui(self):
        ...
        # window: MANO ==================================================================================================
        if self.gaussians.binding is not None and False: # TODO
            with dpg.window(label="MANO parameters", tag="_mano_window", autosize=True, pos=(self.W-300, 0)):
                def callback_enable_control(sender, app_data):
                    if app_data:
                        self.gaussians.update_mesh_by_param_dict(self.mano_param)
                    else:
                        self.gaussians.select_mesh_by_timestep(self.timestep, zero_origin=True)
                    self.need_update = True
                ...
                def callback_set_pose(sender, app_data):
                    ...
                    self.mano_param[joint][0, axis_idx] = app_data
                    if joint == 'eyes':
                        self.mano_param[joint][0, 3+axis_idx] = app_data
                ...
                for joint in ['neck', 'jaw', 'eyes']:
                    if joint in self.mano_param:
                        with dpg.group(horizontal=True):
                            dpg.add_slider_float(min_value=-max_rot, max_value=max_rot, format="%.2f", default_value=self.mano_param[joint][0, 0], callback=callback_set_pose, tag=f"_slider-{joint}-x", width=70)
                            dpg.add_slider_float(min_value=-max_rot, max_value=max_rot, format="%.2f", default_value=self.mano_param[joint][0, 1], callback=callback_set_pose, tag=f"_slider-{joint}-y", width=70)
                            dpg.add_slider_float(min_value=-max_rot, max_value=max_rot, format="%.2f", default_value=self.mano_param[joint][0, 2], callback=callback_set_pose, tag=f"_slider-{joint}-z", width=70)
                ...
                dpg.add_text('   pitch       yaw       roll')
                
                dpg.add_separator()
                
                def callback_set_expr(sender, app_data):
                    expr_i = int(sender.split('-')[2])
                    self.mano_param['expr'][0, expr_i] = app_data
                    if not dpg.get_value("_checkbox_enable_control"):
                        dpg.set_value("_checkbox_enable_control", True)
                    self.gaussians.update_mesh_by_param_dict(self.mano_param)
                    self.need_update = True
                ...
                def callback_reset_mano(sender, app_data):
                    self.reset_mano_param()
                    if not dpg.get_value("_checkbox_enable_control"):
                        dpg.set_value("_checkbox_enable_control", True)
                    self.gaussians.update_mesh_by_param_dict(self.mano_param)
                ...
                dpg.add_button(label="reset MANO", tag="_button_reset_mano", callback=callback_reset_mano)
